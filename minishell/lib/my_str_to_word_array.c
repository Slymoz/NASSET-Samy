...
char **my_tab_path(const char *str, char **tab, int nb_word)
{
    int i = 0;
    int j = 0;
    int k = 0;
    ...
    while (i < nb_word) {
        tab[i] = malloc(my_lenword(str, j) + 1000);
        k = 0;
        while (my_issep_path(str[j]) == 1) {
            tab[i][k] = str[j];
            j = j + 1;
            k = k + 1;
        }
...
char **my_str_to_word_array_path(char const *str)
{
    ...
    if ((tab = malloc(sizeof(char *) * (nb_word + 1000))) == NULL) {
        return NULL;
    }

    tab = my_tab_path(str, tab, nb_word);
    return tab;
}
